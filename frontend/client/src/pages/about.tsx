          ...
          <Grid container sx={{ mt: { xs: 40, sm: 42, md: 45, lg: 48 } }}>
            ...
            >
              ...
              <Box className={styles.heading} ref={ref17}>
                ...
                >
                  一人または複数人でタスク管理できるWebアプリケーションです。
                  タスクを作成や更新、削除する以外にもグループや認証や並び替え、ページネーションなどの機能もあります。
                </Box>
              </Box>
              <Box className={styles.heading} ref={ref10}>
                ...
                >
                  大学の授業グループワークやゼミの課題を管理できるようなアプリを作ってみたらいいなと思ったからです。
                </Box>
              </Box>
            ...
            >
              ...
              <Box className={styles.heading} ref={ref13}>
                ...
                >
                  2:これらの複数環境を同時に立ち上げ、デプロイや移動などの柔軟性を向上させるためDockerを取り入れた。
                </Box>
                <Box
                  sx={{
                    fontFamily: 'serif',
              ...
              <Box className={styles.heading} ref={ref14}>
                ...
                >
                  フロントエンドでは、Reactの魅力を最大限に活用しようとしたところです。SSGやISRを使ったプリレンダリングやcontextを使ったグローバルな状態管理、Next
                  Linkによる高速なページ変移など。
                </Box>
                <Box
                  sx={{
                ...
                >
                  バックエンドでは、userとtaskモデルの間に中間テーブルとしてgroupモデルを作成したことです。これによりuserとtaskに多対多の関係性を築くことと相互参照ができるようになりました。また完成したアプリには直接関係ないのですが、ユーザー情報のモデルをプロフィールとアカウントのモデルに分けたことです。将来アプリを拡大させる上で役に立つと思います。
                </Box>
              </Box>
              <Box className={styles.heading} ref={ref15}>
                ...
                >
                  インフラ周りで苦労することが多かったです。Dockerでnext.jsを立ち上げるとnginxとの関係でホットリロードが効かなくなったり、ISRの際のエンドポイントでエラーが出たりなど。特にAWS
                  ECSにデプロイする時が一番大変でした。ECSのタスクが起動と停止を無限に繰り返していることを知らずに放置していたり、ログが出ないエラー（ネットワークやセキュリティグループの設定ミスなど）に対処したり、APIサーバーなのでヘルスチェックでルートパスにレスポンス200番を返すのにも苦労しました。今でも思い出すとお腹が痛くなります。インフラはコピペや模倣では動かないことが多く、自分に合った構築をしなければならないので本当に知識や理解力が求められていると感じました。
                  あとは、データベースの修正やデザインの設計、状態管理の方法などで苦労しました。
                </Box>
                <Box className={styles.heading} ref={ref16}>
                  ...
                  >
                    次に、グループに参加したユーザーにタスクを割り当てられるサブタスク機能を実装したら、実用性が上がるのではないかと考えているのでやってみたいです。
                  </Box>
                </Box>
