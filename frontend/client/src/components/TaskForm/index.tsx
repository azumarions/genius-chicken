...
const TaskForm: React.FC<Type> = ({ staticCategorys, taskMutate }) => {
  ...
  const newCategory = async () => {
    await fetch(`${process.env.NEXT_PUBLIC_RESTAPI_URL}/api/category/`, {
      ...
      if (res.status === 401) {
        ...
        setSnackPack((prev) => [
          ...prev,
          { message: 'カテゴリーを作成しました！', key: new Date().getTime() },
        ])
      }
    })
    setInputText('')
  ...
  const create = async (e: { preventDefault: () => void }) => {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_RESTAPI_URL}/api/task/`, {
      ...
      if (res.status === 401) {
        ...
        setSnackPack((prev) => [
          ...prev,
          { message: 'タスクを作成しました！', key: new Date().getTime() },
        ])
      }
    })
    setEditTask({
  ...
  const update = async (e: { preventDefault: () => void }) => {
    ...
    await fetch(
      ...
      if (res.status === 401) {
        ...
        setSnackPack((prev) => [
          ...prev,
          { message: 'タスクを更新しました！', key: new Date().getTime() },
        ])
      }
    })
    setEditTask({
  ...
  return (
    <Container component="main">
      <Box
        component="form"
        onSubmit={editTask.id !== 0 ? update : create}
